...
func BuildCredentialsCache(config aws.Config, cacheDir string) CredentialsCache {
	...
	if err != nil {
		slog.With("err", err).Debug("Could not expand cache path, disabling cache")
		return NewNullCredentialsCache()
	}
	...
	if err != nil {
		slog.With("err", err).Debug("Could not fetch credentials for cache prefix, disabling cache")
		return NewNullCredentialsCache()
	}
