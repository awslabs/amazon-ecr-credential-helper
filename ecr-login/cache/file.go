...
func (f *fileCredentialCache) Set(registry string, entry *AuthEntry) {
	slog.With("registry", registry, "service", entry.Service).
	...
	if err != nil {
		slog.With("err", err).Info("Could not save cache")
	}
}
...
func (f *fileCredentialCache) Clear() {
	...
	if err != nil {
		slog.With("err", err).Info("Could not clear cache")
	}
}
...
func (f *fileCredentialCache) init() *RegistryCache {
	...
	if err != nil {
		slog.With("err", err).Info("Could not load existing cache")
		f.Clear()
