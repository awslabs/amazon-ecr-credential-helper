...
func (self ECRHelper) Add(creds *credentials.Credentials) error {
	if shouldIgnoreCredsStorage() {
		self.logger.With("serverURL", creds.ServerURL).
...
func (self ECRHelper) Delete(serverURL string) error {
	if shouldIgnoreCredsStorage() {
		self.logger.With("serverURL", serverURL).
			Warn("Ignoring request to store credentials since AWS_ECR_IGNORE_CREDS_STORAGE env variable is set." +
				"ecr-login does not require 'docker login', and does not support persisting temporary ECR-issued credentials.")
		return nil
	} else {
...
func (self ECRHelper) Get(serverURL string) (string, string, error) {
	...
	if err != nil {
		self.logger.With("err", err, "serverURL", serverURL).Error("error parsing the serverURL")
	...
	if registry.FIPS {
		...
		if err != nil {
			self.logger.With("err", err).Error("Error resolving FIPS endpoint")
			return "", "", credentials.NewErrCredentialsNotFound()
		}
	} else {
	...
	if err != nil {
		self.logger.With("err", err).Error("Error retrieving credentials")
		return "", "", credentials.NewErrCredentialsNotFound()
	}
...
func (self ECRHelper) List() (map[string]string, error) {
	...
	if err != nil {
		self.logger.With("err", err).Error("Error listing credentials")
